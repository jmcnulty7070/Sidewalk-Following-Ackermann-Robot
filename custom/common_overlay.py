...
def draw_steven_overlay(frame_bgr, mask):
    """
    Draws Steven Dade-style overlay:
      - Purple semi-transparent mask for target class (sidewalk)
      - Red polygon for largest contour
      - Blue rectangles for the two halves
      - White centroids (one per half) and white line between them
      - Returns (angle_deg, out_frame)
    Angle convention: +deg = steer right, -deg = steer left
    """
    h, w = mask.shape[:2]
    out = frame_bgr.copy()

    # purple overlay
    purple_layer = np.zeros_like(out, dtype=np.uint8)
    purple_layer[:] = PURPLE
    alpha = 0.35
    mask_3 = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR) // 255
    overlay = (purple_layer * mask_3).astype(np.uint8)
    ...
    dx = p2[0] - p1[0]
    dy = p1[1] - p2[1]  # screen -> math coords
