...
- Robot will blend GPS heading with sidewalk detection  
- It tries to stay centered on sidewalk while generally heading to next waypoint

---

## 🕹️ RC Mode Switching

Set up a 3-position switch on your radio (e.g. channel 9 or 10):
...
- Test vision-only with a saved video:
```bash
...
Create or edit your `segmav.service`:
...
- `lidar_to_mavlink_bridge.py` must be in `segmav_bridge/scripts/` for ROS to find it  
- Build with `catkin_make` anytime you change ROS code
